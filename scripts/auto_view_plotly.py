...
#************************************************************
#a shameless clone of auto_view.py but now with plotly!
#************************************************************
    ...
    for pol in ['xx', 'yy']:
        ...
        try:
            if not np.isnan(amps[(ant, pol)]):
                ...
                '''
                '''
                trace=go.Scatter(x=antpos[ant,0],y=antpos[ant,1]+3*(poli[pol]-0.5),
                                 c=ampS[(ant,pol)],vmin=vmin,vmax=vmax,cmap=goodbad,
                                 text=[text],textposition='bottom')
                '''
                x_good.append(antpos[ant,0])
                y_good.append(antpos[ant,1]+3*(poli[pol]-0.5))
                c_good.append(ampS[(ant,pol)])
                txt_good.append(text)
            else:
                '''
                ...
                '''
                '''
                trace=go.Scatter(antpos[ant,0],antpos[ant,1]+3*(poli[pol]-0.5),
                                 maker='x',color='k',mode='markers+text',
                                 text=[text],textposition='bottom')
                '''
                x_bad.append(antpos[ant,0])
                y_bad.append(antpos[ant,1]+3*(poli[pol]-0.5))
                txt_bad.append(text)
        except KeyError:
            '''
            plt.scatter(antpos[ant, 0], antpos[ant, 1] + 3 * (poli[pol] - 0.5),
                        marker='x', color='k',mode='markers')
            '''
            '''
            trace=go.Scatter(antpos[ant,0],antpos[ant,1]+3*(poli[pol]-0.5),
                             marker='x',color='k',mode='markers',
            ...
            y_bad.append(antpos[ant,1]+3*(poli[pol]-0.5))
            txt_bad.append(text)
trace_good=go.scatter(x_good,y_good,mode='markers',
                      marker=Marker(size=16,cmax=vmax,cmin=vmin,
                                    color=c_good,
                                    colorbar=Colorbar(title=label),
                                    colorscale=goodbad)
                      text=txt_bood,textpositions='top')
trace_bad=go.scatter(x_bad,y_bad,mode='markers',
                     marker=Marker(size=16,color='#000000'),
                     text=txt_bad,textpositions='top')
...
plot_url=py.plot(fig,filename='autos-vs-position')
